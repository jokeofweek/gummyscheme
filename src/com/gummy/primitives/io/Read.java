...
/**
 * This primitive IO procedure reads a Scheme datum from an input port.
 * 
...
public class Read extends Procedure {
	...
	public Object apply(Environment environment, List<Object> arguments) {
		try {
			return Reader.read(Marshall.getInputPort(Expression.eval(arguments.get(0), environment)));
		} catch (IOException e) {
			throw new InterpreterException("Could not read - " + e.getMessage());
		}
	}
